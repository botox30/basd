...
class Farfetch(Brand):
    ...
    async def scrape_web(self) -> dict:
        ...
        json_ld_script = farfetch_data.find('script', type='application/ld+json')
        if json_ld_script:
            try:
        ...
        if not name:
            og_title = farfetch_data.find('meta', property='og:title')
            if og_title and og_title.get('content'):
                name = og_title['content']

        if not image:
            og_image = farfetch_data.find('meta', property='og:image')
        ...
        if not name or not image:
            raise utils.GenerationError("farfetch_scrape")
...
class GrailPoint(Brand):
    ...
    async def scrape_web(self) -> dict:
        ...
        name_element = grailpoint_data.find("h1", class_="single-product__title")
        if not name_element:
            raise utils.GenerationError("grailpoint_url")
        name = name_element.text
        
        # Try to find image
        image_element = grailpoint_data.find('meta', {'property': 'og:image'})
        if not image_element or not image_element.get("content"):
            raise utils.GenerationError("grailpoint_url")
...
def get_config_path():
    import os
    possible_paths = [
        "ReceiptGenBot/receiptgen/config.json",
        "receiptgen/config.json",
        "/root/amethyxbot/ReceiptGenBot/receiptgen/config.json"
    ]
    for path in possible_paths:
        if os.path.exists(path):
            return path
    return "receiptgen/config.json"
